...
function displayAggregated() {
    ...
    for (const reportKey in allExtractedData) {
        ...
        activeMergeRules.forEach(rule => {
            ...
            const targetKey = targetKeys.sort((a,b) => parseInt(a.split('::')[1]) - parseInt(b.split('::')[1]))[0];
        ...
        const finalDataList = Array.from(adjustedLedger.values());
        ...
        const sourceKeysToHide = new Set((mergeRules[reportKey] || []).flatMap(rule => rule.sources));
