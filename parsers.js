...
function _parseFixed(data, config, fundName, sheet, startRow, colMap) {
  ...
  for (let i = startRow; i < data.length; i++) {
    ...
    if (!keyText || keyText.startsWith("註") || keyText.startsWith("附註"))
      continue;
...
function _parseSideBySide(data, config, fundName, sheet) {
  ...
  if (config.subTableIdentifier === "負債") {
    ...
    for (let i = 0; i < data[headerRowIndex].length; i++) {
      if (
        String(data[headerRowIndex][i])
          .replace(/\s|　/g, "")
          .includes(config.keyColumn)
      ) {
        if (foundFirst) {
          tableStartCol = i;
          break;
  ...
  const records = [];
  let dataStartRowIndex = data.findIndex(
    (row) =>
      Array.isArray(row) &&
      String(row[tableStartCol] || "").includes(config.subTableIdentifier),
  );
...
function parseBalanceSheet(data, fundName, sheet) {
  const assetConfig = {
    keyColumn: "科目",
    columns: ["科目", "本年度決算核定數", "上年度決算審定數", "比較增減"],
    subTableIdentifier: "資產",
  };
  const liabilityConfig = {
    keyColumn: "科目",
    columns: ["科目", "本年度決算核定數", "上年度決算審定數", "比較增減"],
    subTableIdentifier: "負債",
  };
  const assetRecords = _parseSideBySide(data, assetConfig, fundName, sheet);
  const liabilityRecords = _parseSideBySide(
    data,
    liabilityConfig,
    fundName,
    sheet,
...
function parseBusinessProfitLoss_Stateful(data, config, fundName, sheet) {
    // 損益表的表頭結構複雜（多列表頭 + 合併儲存格），故直接使用固定映射
    // 根據實際檔案分析，資料列從第 5 行開始 (index 4)
    const startRow = 5; // Fixed: Row 5 (0-indexed = 4, but loop uses 1-indexed display)
...
function parseBusinessAppropriation_Stateful(data, config, fundName, sheet) {
    // 盈虧撥補表的表頭結構複雜（多列表頭 + 合併儲存格），故直接使用固定映射
    const startRow = 5; // Fixed: 與損益表相同結構
...
function parseFixedBusinessCashFlow(data, config, fundName, sheet) {
    ...
    // 使用 getHeaderMapping dynamic mapping 或是 fallback 到固定 mapping
    // 因為有的檔案可能欄位名稱有微小差異，但這裡是 Fixed parser，我們可以用 mix 策略
    // 優先嘗試動態抓取
    let colMap = getHeaderMapping(data[headerRowIndex], config.columns);
    
    // 檢查關鍵欄位是否存在，若不存在則嘗試預設的固定 mapping (針對某些欄位名可能被修飾的情況)
    if (colMap[config.keyColumn] === undefined) {
         colMap = { '項目': 0, '本年度預算數': 1, '原列決算數': 2, '修正數': 3, '決算核定數': 4 };
    }

    const records = [];
    const keyColIndex = colMap[config.keyColumn];
    if (keyColIndex === undefined) return []; // 仍無法確定 Key Column，放棄
...
function parseBusinessBalanceSheet_SideBySide(data, config, fundName, sheet) {
    // 資產負債表的表頭結構複雜，使用固定映射
    const startRow = 5; // Fixed: 與其他報表相同結構

    const assetConfig = { ...config };
    const assetColMap = { '科目': 3, '上年度決算數': 1, '原列決算數': 4, '修正數': 5, '決算核定數': 6 };
    const assetRecords = _parseFixed(data, assetConfig, fundName, sheet, startRow, assetColMap);
    const liabilityConfig = { ...config };
    const liabilityColMap = { '科目': 10, '上年度決算數': 8, '原列決算數': 11, '修正數': 12, '決算核定數': 13 };
    const liabilityRecords = _parseFixed(data, liabilityConfig, fundName, sheet, startRow, liabilityColMap);
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        let fundName =
          extractFundName(workbook, selectedFundType) ||
        ...
        resolve(
          fileHasData
            ? { fundName, fileName: file.name, data: extractedData }
            : null,
        );
      } catch (error) {
